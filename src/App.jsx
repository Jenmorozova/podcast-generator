...
function App() {
      ...
      if (voice.provider === 'ElevenLabs') {
        ...
        try {
          ...
          audio.oncanplay = () => {
            ...
            audio.play().catch(error => {
              ...
              console.log('Не удалось воспроизвести аудио от ElevenLabs.')
            })
          }
          ...
          audio.onerror = (error) => {
            ...
            URL.revokeObjectURL(audioUrl)
            console.log('Ошибка воспроизведения аудио от ElevenLabs.')
          }
          
        } catch (error) {
          setIsListening(false)
          console.error('❌ Ошибка ElevenLabs API:', error)
          console.log('Ошибка при обращении к ElevenLabs API. Проверьте подключение к интернету.')
        }
        ...
        if (systemVoice) {
          ...
          utterance.onerror = (event) => {
            ...
            console.log('Ошибка воспроизведения. Попробуйте другой голос.')
          }
          ...
          setIsListening(false)
          console.log('Системные голоса не найдены. Попробуйте перезагрузить страницу.')
        }
        ...
        if (!yandexApiKey) {
          ...
          console.log('Для использования голосов Яндекса нужен API ключ. Введите его в поле ниже.')
          return
        }
        ...
        try {
          ...
          audio.onerror = (error) => {
            ...
            URL.revokeObjectURL(audioUrl)
            console.log('Ошибка воспроизведения аудио от Яндекса.')
          }
          
        } catch (error) {
          setIsListening(false)
          console.error('❌ Ошибка запроса к Яндекс API:', error)
          console.log(`Ошибка API Яндекса: ${error.message}\n\nПроверьте правильность API ключа.`)
        }
        
      } else {
        // Для остальных провайдеров показываем сообщение
        setIsListening(false)
        console.log(`Голос "${voice.name}" требует настройки API ключей.\n\nПока используйте:\n• Системные голоса (работают всегда)\n• Google Translate (бесплатно)\n• Яндекс (с API ключом)`)
      }
      
    } catch (error) {
      setIsListening(false)
      console.error('❌ Ошибка внешнего TTS:', error)
      console.log('Ошибка при воспроизведении через внешний сервис.')
    }
  }
  ...
  const listenToText = () => {
    if (!script.trim()) {
      console.log('Введите текст для прослушивания!')
      return
    }

    if (!isInitialized) {
      console.log('Голоса еще загружаются. Подождите немного и попробуйте снова.')
      return
    }
    ...
    if (!window.speechSynthesis) {
      console.log('Синтез речи не поддерживается в этом браузере')
      return
    }
    ...
    setTimeout(() => {
      ...
      try {
        ...
        setIsListening(false)
        console.log('Не удалось запустить воспроизведение. Попробуйте еще раз.')
      }
  ...
  const refreshVoices = () => {
    ...
    const loadVoices = () => {
      ...
      if (russianVoices.length > 0) {
        console.log(`✅ Обновлено! Найдено ${russianVoices.length} русских голосов: ${russianVoices.map(v => v.name).join(', ')}`)
      } else {
        console.log('⚠️ Русские голоса не найдены. Следуйте инструкциям ниже для их установки.')
      }
    }
  ...
  const testRussianVoices = async () => {
    ...
    if (russianVoices.length === 0) {
      console.log('Русские голоса не найдены! Установите их по инструкции ниже.')
      return
    }
    ...
    console.log(`Тестирование завершено! Проверьте консоль (F12) для результатов.`)
  }
  ...
  const generatePodcast = async () => {
    if (!script.trim()) {
      console.log('Введите текст для генерации подкаста!')
      return
    }
...
• Громкость: ${Math.round(volume * 100)}%`
      ...
      setIsGenerating(false)
      console.log('Ошибка при генерации подкаста: ' + error.message)
    }
  }
