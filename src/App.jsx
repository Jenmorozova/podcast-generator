...
function App() {
  ...
  useEffect(() => {
    const loadVoices = () => {
      ...
      if (russianVoices.length > 0) {
        ...
        console.log('ℹ️ Русские голоса не обнаружены')
        setVoices([])
  ...
  const playExternalTTS = async (text, voice) => {
    ...
    try {
      ...
      if (voice.provider === 'ElevenLabs') {
        ...
        if (systemVoice) {
          ...
          utterance.onerror = (event) => {
            ...
            console.log('🔄 Воспроизведение недоступно, переключаемся на другой голос')
          }
          ...
          setIsListening(false)
          console.log('🔄 Системные голоса недоступны, перезагрузите страницу')
        }
        ...
        if (!yandexApiKey) {
          ...
          console.log('ℹ️ Для голосов Яндекса требуется API ключ')
          return
        }
        ...
        try {
          ...
          audio.onerror = (error) => {
            ...
            URL.revokeObjectURL(audioUrl)
            console.log('🔄 Яндекс аудио недоступно, переключаемся на системный голос')
          }
          
        } catch (error) {
          setIsListening(false)
          console.error('❌ Ошибка запроса к Яндекс API:', error)
          console.log(`🔄 API Яндекса недоступен: ${error.message}`)
        }
      ...
      console.log('🔄 Внешний сервис недоступен, переключаемся на системный голос')
    }
  }
  ...
  const listenToText = () => {
    if (!script.trim()) {
      console.log('ℹ️ Введите текст для прослушивания')
      return
    }

    if (!isInitialized) {
      console.log('ℹ️ Голоса загружаются, подождите немного')
      return
    }
    ...
    if (!window.speechSynthesis) {
      console.log('ℹ️ Синтез речи недоступен в этом браузере')
      return
    }
    ...
    setTimeout(() => {
      ...
      utterance.onerror = (event) => {
        ...
        switch(event.error) {
          ...
          case 'audio-busy':
            ...
            break
          case 'audio-hardware':
            errorMessage = 'Проверьте настройки звука.'
            break
          case 'network':
      ...
      try {
        ...
        setIsListening(false)
        console.log('🔄 Запуск воспроизведения, перезапускаем')
      }
  ...
  const refreshVoices = () => {
    ...
    const loadVoices = () => {
      ...
      if (russianVoices.length > 0) {
        ...
        console.log('ℹ️ Русские голоса не обнаружены, используйте доступные голоса')
      }
    }
  ...
  const testRussianVoices = async () => {
      ...
      await new Promise((resolve) => {
        ...
        utterance.onerror = (event) => {
          if (event.error === 'interrupted') {
            ...
            console.log(`🔄 Голос ${voice.name} недоступен: ${event.error}`)
          }
          resolve()
        }
    ...
    console.log(`✅ Тестирование завершено! Результаты в консоли (F12)`)
  }
  ...
  const generatePodcast = async () => {
    if (!script.trim()) {
      console.log('ℹ️ Введите текст для генерации подкаста')
      return
    }
    ...
    try {
      ...
      try {
        ...
        // Fallback: используем системный голос
        console.log('🔄 Переключаемся на системный голос для воспроизведения')
      ...
      setIsGenerating(false)
      console.log('🔄 Генерация подкаста: ' + error.message)
    }
  }
